...
public class UETextureStitcher {
    ...
    private static List<Identifier> rawRegolithTexture = new ArrayList<>();
    private static Map<IOreType, List<Identifier>> rawStoneTexture = new HashMap<>();
        ...
        for (BlockGeneratorHelper helper : BlockGeneratorReference.ROCK_TYPES) {
            for (BlockGeneratorHelper.Entry entry : helper.getEntries()) {
                ...
                if (form == Forms.REGOLITH) {
                    rawRegolithTexture.add(new Identifier(Unearthed.MOD_ID, "textures/block/" + entry.getId() + ".png"));
                } else if (form == Forms.OVERGROWN_ROCK) {
                    rawRegolithTexture.add(new Identifier(namespace, "textures/block/" + stone + ".png"));
                } else if (form instanceof Forms.OreForm) {
                    rawStoneTexture.get(((Forms.OreForm) form).getOreType())
                            .add(new Identifier(namespace, "textures/block/" + stone + ".png"));
                }
            }
        }
    }
...
//                    Files.copy(Thread.currentThread().getContextClassLoader().getResourceAsStream("/assets/pack.png"), new File(RESOURCE_PACK_FOLDER, "pack.png").toPath());
    ...
    public UETextureStitcher prepare() {
        grassSide = readImage(new Identifier(Unearthed.MOD_ID, "textures/block/grass_block_side_overlay.png"));
        grassSnowy = readImage(new Identifier(Unearthed.MOD_ID, "textures/block/grass_block_snow.png"));
        grassSide = readImage(new Identifier(Unearthed.MOD_ID, "textures/block/grass_block_side_overlay.png"));
        for (IOreType oreType : VanillaOreTypes.values()) {
            oreMasks.put(oreType, readImage(new Identifier(Unearthed.MOD_ID, "textures/block/ore/" + oreType.getName() + "_ore_mask.png")));
    ...
    private BufferedImage readImage(Identifier location) {
        try {
    ...
    public static class TexturePair {
        ...
        public TexturePair(Identifier loc, BufferedImage image) {
            this.loc = loc;
            this.image = image;
        }
