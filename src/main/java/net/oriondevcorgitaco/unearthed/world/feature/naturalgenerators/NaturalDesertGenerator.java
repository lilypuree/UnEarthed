...
public class NaturalDesertGenerator extends ChunkCoordinatesFeature<DefaultFeatureConfig> {
    ...
    private void getHighestNoisePoint(double noise) {
        if (noise > storedNoiseHigh) {
            ...
            Unearthed.LOGGER.debug("Highest Noise point: " + storedNoiseHigh);
        }
        if (noise < storedNoiseLow) {
            storedNoiseLow = noise;
            Unearthed.LOGGER.debug("Lowest Noise point: " + storedNoiseLow);
        }
    }
    ...
    public boolean generate(StructureWorldAccess world, Random random, Chunk chunk, int x, int z, DefaultFeatureConfig config) {
        ...
        for (int y = 0; y < topY; y++) {
            ...
            double selectorNoiseValue = selectorNoise.GetValue(z, mutable.getY(), x) * 12 + 0.5;
