...
public class ColorHelper {
    ...
    public static int shiftColorHSL(final int color, float hueChange, float saturationChange, float luminanceChange) {
        ...
        return toInt(AHSLtoARGB(new float[]{hsl[0], (hsl[1] + hueChange) % 360, Mth.clamp(hsl[2] + saturationChange, 0, 1), Mth.clamp(hsl[3] + luminanceChange, 0, 1)}));
    }
    ...
    public static int shiftColorHSV(final int color, float hueChange, float saturationChange, float valueChange) {
        ...
        return toInt(AHSVtoARGB(new float[]{hsv[0], (hsv[1] + hueChange) % 360, Mth.clamp(hsv[2] + saturationChange, 0, 1), Mth.clamp(hsv[3] + valueChange, 0, 1)}));
    }
}
